...
3. Install the Node.js packages with `npm install`.
4. Copy the `.env.example` file to `.env` and fill with your domain, own database and mail details.
5. Ensure the `storage`, `bootstrap/cache` & `public/uploads` folders are writable by the web server ([More information here](/docs/admin/filesystem-permissions/)).
6. In the application root, Run `php artisan key:generate` to generate a unique application key.
7. Build the public JS and CSS assets by running `npm run production`.
8. If not using Apache or if `.htaccess` files are disabled you will have to create some URL rewrite rules as shown below.
9. Set the web root on your server to point to the BookStack `public` folder. This is done with the `root` setting on Nginx or the `DocumentRoot` setting on Apache.
10. Run `php artisan migrate` to update the database.
